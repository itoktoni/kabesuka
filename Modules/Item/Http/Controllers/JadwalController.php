...
use Modules\Item\Dao\Repositories\JadwalRepository;
...
class JadwalController extends Controller
{
    ...
    public function __construct(JadwalRepository $model, SingleService $service)
    {
    ...
    {
        $shift = Shift::where('shift_start', '>=', date('Y-m-d'))->get()->mapWithKeys(function($item){
            return [$item->shift_id => $item->shift_name.' - '.$item->shift_start->format('Y-m-d').' - '.$item->shift_end->format('Y-m-d')];
        })->prepend('- Select Shift -', '');
