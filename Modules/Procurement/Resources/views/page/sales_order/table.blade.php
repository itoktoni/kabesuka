<table id="transaction" class="table table-no-more table-bordered table-striped">
    ...
    <tbody class="markup">
        ...
        <tr>
            <td data-title="ID Product">
                ...
                @else
                <a id="delete" master="{{ $item->so_detail_so_code }}" value="{{ $item->so_detail_product_id }}" href="{{ route(config('module').'_delete') }}" class="btn btn-danger btn-xs btn-block delete-{{ $item->so_detail_product_id }}">
                    Delete
                </a>
                @endif
                <input type="hidden" value="{{ $item['temp_id'] ?? $item->so_detail_product_id }}" name="temp_id[]">
                <input type="hidden" value="{{ $item['temp_id'] ?? $item->so_detail_product_id }}" name="detail[{{ $loop->index }}][temp_id]">
            </td>
            ...
            <td data-title="Qty" class="text-right col-lg-1">
                <input type="text" tabindex="{{ $loop->iteration }}1" name="detail[{{ $loop->index }}][temp_qty]" class="form-control input-sm text-right number temp_qty" value="{{ $item['temp_qty'] ?? $item->so_detail_qty }}">

            </td>
            <td data-title="Price" class="text-right col-lg-1">
                <input type="text" tabindex="{{ $loop->iteration }}2" name="detail[{{ $loop->index }}][temp_price]" class="form-control input-sm text-right money temp_price" value="{{ $item['temp_price'] ?? $item->so_detail_price }}">

            </td>
            <td data-title="Total" class="text-right col-lg-1">
                <input type="text" readonly name="detail[{{ $loop->index }}][temp_total]" class="form-control input-sm text-right number temp_total" value="{{ $item['temp_total'] ?? $item->so_detail_total }}">
            </td>
        </tr>
        @endforeach
        @endisset
    ...
    <tbody class="discount_container">
        <tr>
            ...
            <td data-title="" class="text-right">
                {!! Form::text('so_sum_product', $model->mask_value, ['id' => 'before_discount',
