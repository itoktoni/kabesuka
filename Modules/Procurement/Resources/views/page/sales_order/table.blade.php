<table id="transaction" class="table table-no-more table-bordered table-striped">
		...
		<tr>
			<td data-title="ID Product">
				...
				<button id="delete" value="{{ $item['temp_id'] }}" type="button"
					class="btn btn-danger btn-xs btn-block">Delete {{ $item['temp_id'] }}</button>
				@else
				<a id="delete" master="{{ $item->so_detail_so_code }}" value="{{ $item->so_detail_product_id }}"
					href="{{ route(config('module').'_delete') }}"
					class="btn btn-danger btn-xs btn-block delete-{{ $item->so_detail_product_id }}">
					Delete
				</a>
				@endif
				<input type="hidden" value="{{ $item['temp_id'] ?? $item->so_detail_product_id }}" name="temp_id[]">
				<input type="hidden" value="{{ $item['temp_id'] ?? $item->so_detail_product_id }}"
					name="detail[{{ $loop->index }}][temp_id]">
			</td>
			<td data-title="Product">
				<input type="text" readonly class="form-control input-sm"
					value="{{ $item['temp_product'] ?? $item->has_product->product_name ?? '' }}"
					name="detail[{{ $loop->index }}][temp_product]">
			</td>
			<td data-title="Description">
				<textarea rows="2" readonly class="form-control input-sm"
					name="detail[{{ $loop->index }}][temp_desc]">{{ $item['temp_desc'] ?? $item->has_product->product_description }}</textarea>
			</td>
			<td data-title="Qty" class="text-right col-lg-1">
				<input type="text" tabindex="{{ $loop->iteration }}1" name="detail[{{ $loop->index }}][temp_qty]"
					...
					value="{{ $item['temp_qty'] ?? $item->so_detail_qty }}">
			...
			<td data-title="Price" class="text-right col-lg-1">
				<input type="text" tabindex="{{ $loop->iteration }}2" name="detail[{{ $loop->index }}][temp_price]"
					...
					value="{{ $item['temp_price'] ?? $item->so_detail_price }}">
			...
			<td data-title="Total" class="text-right col-lg-1">
				<input type="text" readonly name="detail[{{ $loop->index }}][temp_total]"
					...
					value="{{ $item['temp_total'] ?? $item->so_detail_total }}">
			</td>
		</tr>
		@endforeach
		@endisset
    ...
    <tbody>
		<tr>
			...
			<td data-title="" class="text-right">
				{!! Form::text('', $model->mask_total ?? 0 , ['readonly', 'class' => 'number form-control text-right'])
				!!}
			</td>
		</tr>
	</tbody>
