...
class ReportSalesDetail extends SalesRepository implements FromView, WithColumnFormatting, WithColumnWidths, GenerateReport
{
    public $name;
    ...
    public function data()
    {
        ...
        if ($from = request()->get('from')) {
            $query->whereDate('so_date_order', '>=', $from);
        }

        if ($to = request()->get('to')) {
            $query->whereDate('so_date_order', '<=', $to);
        }
