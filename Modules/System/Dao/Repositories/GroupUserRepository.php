...
class GroupUserRepository extends GroupUser implements CrudInterface
{
    ...
    // public function __construct(GroupModule $group_module, GroupUserConnectionGroupModule $group_user_connection_group_module)
    // {
    //     self::$group_module = $group_module;
    //     self::$group_user_connection_group_module = $group_user_connection_group_module;
    // }
    ...
    public function getGroupByUser($user){
        $select = DB::table(GroupModuleFacades::getTable());
        $select->join(GroupUserConnectionGroupModuleFacades::getTable(), GroupModuleFacades::getKeyName(), '=', 'conn_gu_group_module');
    ...
    public function saveConnectionModule($code, $data)
    {
        try {
            DB::beginTransaction();
            DB::table(GroupUserConnectionGroupModuleFacades::getTable())->where('conn_gu_group_user', '=', $code)->delete();
            if (!empty($data)) {
            ...
            DB::table(GroupUserConnectionGroupModuleFacades::getTable())->insert($insert);
            DB::commit();
