...
class PublicController extends Controller
{
    ...
    {
        $sosmend = Helper::createOption((new SosmedRepository()), false, true, true);
        $page = Helper::createOption((new PageRepository()), false, true, true);
        view()->share('public_page', $page);
        view()->share('public_sosmed', $sosmend);
    }
    ...
    public function index()
    {
        ...
        return view('frontend.kababi.page.index')->with([
    ...
    public function booking(BookingRequest $request, BookingCreateService $service, BookingRepository $repository){
        ...
        if(isset($data['status']) && $data['status']){
            ...
            return redirect()->route('payment', ['code' => $code]);
        }
    }

    public function payment($code){
        $model = Booking::where('booking_code', $code)->first();

        if(request()->get('check') == 'qris'){

            if(isset($model) && $model->booking_qris_status != 'paid'){
                ...
                if($check_qris->status == 'success'){
                    $model->update([
                        ...
                        'booking_status' => BookingType::Booked,
                    ]);
                }
                else{
                    $model->update([
        ...
        return view('frontend.kababi.page.payment')->with([
            'model' => $model
        ]);
    }
    ...
    public function page($slug = false)
    {
        if ($slug) {
            ...
            return View('frontend.kababi.page.page')->with([
                'data' => $data,
            ]);
        }
    ...
    {
        // return View(PluginsHelper::setViewFrontend(__FUNCTION__))->with([]);
    }
